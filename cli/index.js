...
async function execute({ _optionValues }) {
  ...
  if (options.summary) {
    ...
    buildSummaryReport(options, reports);
  }

  console.log('Finish');
}

function buildSummaryReport(options, reports) {
  const out = path.resolve(__dirname, options.out || OUT);
  ...
  return buildHtml(summary, options, htmlPath);
}

function buildHtml(summary, options, htmlPath) {
  let head =
  ...
  if (summary.desktopResults.length > 0) {
    ...
    body += `<div class="my-3">${borderTableHead}<tr><td>Url</td><td>Filename</td><td>Performance</td><td>Accessibility</td><td>Best-Pratices</td><td>SEO</td>${
      options.threshold ? '<td>Warning</td>' : ''
    }</tr>`;
  ...
  console.log('Html Report Finish Build');
